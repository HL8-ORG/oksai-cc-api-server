...
3. **核心实体简化**: 从 167 个实体简化到 17 个核心实体（以 `03-核心实体清单.md` 为准）
4. **第三方认证**: 添加 Microsoft 认证，确保 Google、GitHub、Auth0 正常工作
5. **高可测试性**: 建立完整的单元测试、集成测试、E2E 测试体系

### 1.2 与项目宪章一致性说明（必须遵守）

本计划严格对齐 `.cursor/rules/AGENTS.md` 第一阶段目标（19-41）。如与本计划存在差异，以项目宪章为准：

- **认证删减**：移除 facebook、twitter、linkedin、keycloak、fiverr 等，仅保留 Google、Microsoft、GitHub、Auth0。
- **ORM 简化**：仅使用 MikroORM，移除 TypeORM 与多 ORM 抽象层，不再支持多 ORM。
- **数据库模型简化**：默认 PostgreSQL，后续扩展 MongoDB、Better-SQLite。
- **插件系统保留**：系统插件（必备、受保护）+ 业务插件（可选、可热拔插）。
- **通用能力建设**：身份认证与授权、多租户管理、权限管理、审计跟踪、系统配置、系统监控、系统日志。
- **结构对齐与复用**：新开发代码组织结构对齐 `qauzy-backup`，优先复用旧实现，避免重复造轮子。
