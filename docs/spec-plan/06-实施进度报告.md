...
| 任务 6: 配置与构建优化      | ✅ 已完成 | 100% | MikroORM v6 配置、构建与 lint 基线可用、应用可启动              |
| 任务 5: 测试策略实施        | 🟡 进行中 | 40%  | 已新增单元测试与 E2E 骨架，覆盖率目标与关键路径仍需补齐         |
| 任务 7: 文档与开发指引完善  | 🟡 进行中 | 60%  | spec-plan 文档集已建立，持续对齐代码现状与项目宪章目标           |

**总体进度**: 进行中（建议以任务状态表与 CI 指标为准）
...
1. **对齐 MikroORM 官方示例项目配置**

    - 文件：`apps/base-api/src/config/mikro-orm.config.ts`
    - 参考 `guides/nestjs-realworld-example-app` 最佳实践
    - 使用 `driver: PostgreSqlDriver` 配置（MikroORM v6 要求）
    - 直接导入 `PostgreSqlDriver` 从 `@mikro-orm/postgresql`
    - 使用 `forRoot()` 在根模块注册配置
    - 使用 `forFeature()` 在子模块注册实体
    - 移除抽象基类在 `entities` 数组中的注册（符合官方规范）

2. **修复 MikroORM v6 实体验证**

    - 移除抽象基类属性上的 `@Index()` 装饰器： - `TenantBaseEntity`: 移除 `tenantId` 上的索引 - `TenantOrganizationBaseEntity`: 移除 `organizationId` 上的索引
    - 移除子类中继承属性的索引定义： - `FeatureOrganization`: 移除 `featureId` 上的索引 - `Organization`: 移除 `status` 上的索引
    - 移除所有属性级别和实体级别的索引定义： - `AnalyticsReport`: 移除 `createdBy` 上的索引
    - 添加缺失实体到 `entities` 数组： - `AnalyticsReport`: 从 core 移到 organization 包
    - 修正 `Feature` 的导入方式（从 type-only 改为直接导入）

3. **配置数据库连接**

    - 文件：`apps/base-api/.env`
    - 修正数据库密码：`postgres`
    - 修正数据库名称：`oksai`
    - 验证 PostgreSQL 数据库连接正常

4. **配置 Redis 和环境变量**

    - 文件：`apps/base-api/.env`
    - 修正 Redis 端口：`6379`（避免端口冲突）
    - 添加 Redis 禁用配置：`REDIS_ENABLED=false`

5. **解决 TypeScript 编译问题**

    - 文件：`libs/bootstrap/tsconfig.lib.json`
    - 添加类型包：`@types/express-session`
    - 修复编译类型错误
...
6. **应用启动成功**
    ...
    - 数据库连接成功
    - 所有模块加载成功
    - 应用成功启动并监听在 `http://localhost:3000`
