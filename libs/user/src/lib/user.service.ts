...
export class UserService {
	 ...
	 * @returns 已创建的用户
	 * @throws ForbiddenException 当尝试在不同租户下创建用户时
	 ...
	 * ```
	 */
	async create(createUserDto: CreateUserDto, currentTenantId: string): Promise<User> {
		if (createUserDto.tenantId !== currentTenantId) {
			throw new ForbiddenException('无法在不同租户下创建用户');
		}
	 ...
	 * ```
	 */
	async findAll(query: QueryUserDto, currentTenantId: string): Promise<{ data: User[]; total: number }> {
	 ...
	 * @example
	 * ```typescript
	 * const user = await userService.findOne('user-123', 'tenant-123');
	 * ```
	 */
	async findOne(id: string, currentTenantId: string): Promise<User> {
	 ...
	 * ```
	 */
	async update(id: string, updateUserDto: UpdateUserDto, currentTenantId: string): Promise<User> {
		const user = await this.findOne(id, currentTenantId);
	 ...
	 * @example
	 * ```typescript
	 * await userService.remove('user-123', 'tenant-123');
	 * ```
	 */
	async remove(id: string, currentTenantId: string): Promise<void> {
		const user = await this.findOne(id, currentTenantId);
	 ...
	 * ```
	 */
	async updateAvatar(id: string, updateAvatarDto: UpdateAvatarDto, currentTenantId: string): Promise<User> {
		const user = await this.findOne(id, currentTenantId);
	 ...
	 * ```
	 */
	async updatePassword(id: string, updatePasswordDto: UpdatePasswordDto, currentTenantId: string): Promise<void> {
		const user = await this.findOne(id, currentTenantId);
	 ...
	 * @example
	 * ```typescript
	 * const user = await userService.deactivate('user-123', 'tenant-123');
	 * ```
	 */
	async deactivate(id: string, currentTenantId: string): Promise<User> {
		const user = await this.findOne(id, currentTenantId);
	 ...
	 * @example
	 * ```typescript
	 * const user = await userService.activate('user-123', 'tenant-123');
	 * ```
	 */
	async activate(id: string, currentTenantId: string): Promise<User> {
		const user = await this.findOne(id, currentTenantId);
	 ...
	 * @example
	 * ```typescript
	 * await userService.updateLastLogin('user-123', 'tenant-123');
	 * ```
	 */
	async updateLastLogin(id: string, currentTenantId: string): Promise<void> {
		const user = await this.findOne(id, currentTenantId);
