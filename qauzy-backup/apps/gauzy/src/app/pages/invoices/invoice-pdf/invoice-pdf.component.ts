...
@Component({
	...
	template: `@if (fileURL) {
		<iframe type="application/pdf" id="iframe" class="pdfDoc" [src]="fileURL | safeUrl" frameBorder="0"></iframe>
		} @if (isLoading) {
		<div [nbSpinner]="isLoading" nbSpinnerStatus="primary" nbSpinnerSize="large" class="pdfDoc loading"></div>
		} @if (error) {
