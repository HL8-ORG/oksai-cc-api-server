...
function renderBackgroundLayer(isTimerStopped = true): void {
	...
	const backgroundColor = isTimerStopped ? currentThemeColors.stopped.bgColor : currentThemeColors.active.bgColor;
	...
	backgroundContext.roundRect(CANVAS_START_X, CANVAS_START_Y, ICON_AREA_WIDTH, canvasHeight, [
		BORDER_RADIUS,
		0,
		0,
		BORDER_RADIUS
...
function renderTrayIconWithTime(currentTime?: string): string {
	...
	const textColor = isTimerActive ? currentThemeColors.active.txtColor : currentThemeColors.stopped.txtColor;
	...
	if (loadedIconImage) {
		...
		mainContext.drawImage(loadedIconImage, iconPositionX, iconPositionY, TRAY_ICON_SIZE, TRAY_ICON_SIZE);
	}
