...
When('User enter filter input field value', () => {
	organizationTagsUserPage.enterFilterInputData(OrganizationTagsPageData.editTagName);
});

Then('User can see filtered tag', () => {
	organizationTagsUserPage.filteredTagVisible(OrganizationTagsPageData.editTagName);
});
