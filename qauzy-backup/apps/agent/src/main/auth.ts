...
import { LocalStore, DesktopDialog, TranslateService } from '@gauzy/desktop-lib';
...
async function handleCloseAuthWindow(appWindow: AppWindow) {
	...
	if (!authConfig?.token) {
		...
		dialog.options.buttons = [TranslateService.instant('BUTTONS.CANCEL'), TranslateService.instant('BUTTONS.EXIT')];
