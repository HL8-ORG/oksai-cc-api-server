...
class PushActivities {
	...
	async saveImage(recordedAt: string, images: string[], timeSlotId?: string) {
		try {
			...
			await Promise.all(
				images.map((imageTemp) => this.uploadCapturedImage(auth, recordedAt, imageTemp, timeSlotId))
			);
		} catch (error) {
	...
	getActivities(activities: KbMouseActivityTO, duration: number, auth: TAuthConfig): TParamsActivities[] {
		return [...this.getKeyboardActivities(activities, duration, auth), ...this.getActiveWindows(activities, auth)];
	}
	...
	timeSlotParams(activities: KbMouseActivityTO): TTimeSlot | undefined {
		...
		const projectConfig = getProjectConfig();
		const overall = this.getDurationOverAllSeconds(
			new Date(activities.timeStart),
			new Date(activities.timeEnd),
			activities.afkDuration
		);
		...
		return {
			...
			...(projectConfig?.organizationContactId
				? { organizationContactId: projectConfig?.organizationContactId }
				: {})
		};
	}
	...
	public async syncTimer(job: ITimerCallbackPayload) {
		...
		if (timerLocal?.id) {
			...
			if (
				job.data?.isStopped &&
				timerLocal?.stoppedAt &&
				timerLocal?.timelogId &&
				this.currentSessionTimeLogId !== timerLocal.timelogId &&
				!timerLocal?.synced
			) {
				try {
	...
	public async syncTimeSlot(job: ITimeslotQueuePayload) {
		...
		if (timeSlotLocal?.id) {
			...
			if (resp?.id || timeSlotLocal?.timeslotId) {
				const images =
					typeof timeSlotLocal.screenshots === 'string'
						? (() => {
								try {
