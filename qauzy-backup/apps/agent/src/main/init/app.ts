...
import { AppError, LocalStore, DesktopThemeListener, ProviderFactory, TranslateService } from '@gauzy/desktop-lib';
import { getApiBaseUrl, getAuthConfig, getTrayIcon, getAppSetting } from '../util';
...
export async function startServer(value: any) {
	...
	trayMenu = TrayMenu.getInstance(getTrayIcon(), true, { helpSiteUrl: environment.COMPANY_SITE_LINK });
	trayMenu.build();
...
function setVariableGlobal(configs: { serverUrl?: string; port?: number; isLocalServer?: boolean }) {
