...
export class DesktopQueue {
	...
	private auditQueue(name: TQueueName, que: Queue) {
		que.on('task_queued', (id: string, info: any) =>
			this.auditQueueCallback?.({
				id,
				data: info,
				type: 'queued',
				queue: name
		...
		que.on('task_started', (id: string, info: any) =>
			this.auditQueueCallback?.({
				id,
				type: 'running',
				queue: name,
		...
		que.on('task_finish', (id: string, info: any) =>
			this.auditQueueCallback?.({
				id,
				queue: name,
				type: 'succeeded',
		...
		que.on('task_progress', (id: string, info) =>
			this.auditQueueCallback?.({
				id,
				queue: name,
				type: 'progress',
		...
		que.on('task_failed', (id: string, err) =>
			this.auditQueueCallback?.({
				id,
				queue: name,
				type: 'failed',
	...
	public initTimerQueue(timerCallback: (job: ITimerCallbackPayload, cb: (err?: any) => void) => void) {
		if (!this.timerQueue) {
			this.timerQueue = new Queue(timerCallback, {
		...
		if (!this.timerQueueRetry) {
			this.timerQueueRetry = new Queue(timerCallback, {
				id: (task, cb) => cb(null, `timer-queue:${task.timerId}`),
				concurrent: 1,
				maxRetries: 20,
				retryDelay: 15 * 1000,
	...
	public initTimeslotQueue(timeSlotCallback: (job: ITimeslotQueuePayload, cb: (err?: any) => void) => void) {
		if (!this.timeSlotQueue) {
			this.timeSlotQueue = new Queue(timeSlotCallback, {
		...
		if (!this.timeSlotQueueRetry) {
			this.timeSlotQueueRetry = new Queue(timeSlotCallback, {
				id: (task, cb) => cb(null, `time-slot-queue-${task.activityId}`),
				concurrent: 2,
				maxRetries: 20,
				retryDelay: 15 * 1000,
	...
	public initScreenshotQueue(screenshotCallback: (job: IScreenshotQueuePayload, cb: (err?: any) => void) => void) {
		if (!this.screenshotQueue) {
			this.screenshotQueue = new Queue(screenshotCallback, {
		...
		if (!this.screenshotQueueRetry) {
			this.screenshotQueueRetry = new Queue(screenshotCallback, {
				id: (task, cb) => cb(null, `screenshot-queue-${task.screenshotId}`),
				concurrent: 2,
				maxRetries: 20,
				retryDelay: 15 * 1000,
