...
export class KbMouseTimer {
	...
	private onFlushCallback: ((timeData: TActivityProcessParam) => void) | null = null;
	...
	private getTimeNextScreenshot() {
		if (this.randomScreenshotInterval) {
			const newDuration =
				DURATION_SCREENSHOT_LIST[Math.floor(Math.random() * DURATION_SCREENSHOT_LIST.length)] * 60;
