...
import { ISidebarActionConfig, ISidebarConfig, NavigationBuilderService } from './navigation-builder.service';
 ...
 */
export function addSidebarActionItem(config: ISidebarActionConfig): EnvironmentProviders {
	return provideAppInitializer(() => {
		const initializerFn = ((navigationBuilderService: NavigationBuilderService) => () => {
 ...
 */
export function registerSidebarWidget(id: string, config: ISidebarConfig): EnvironmentProviders {
	return provideAppInitializer(() => {
		const initializerFn = ((navigationBuilderService: NavigationBuilderService) => () => {
