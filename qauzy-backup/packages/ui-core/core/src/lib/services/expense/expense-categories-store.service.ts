...
export class ExpenseCategoriesStoreService {
	private _expenseCategories$: BehaviorSubject<IExpenseCategory[]> = new BehaviorSubject([]);
	public expenseCategories$: Observable<IExpenseCategory[]> = this._expenseCategories$.asObservable();
		...
		return this.expenseCategoriesService
			...
			.pipe(
				tap((category: IExpenseCategory) =>
					this._expenseCategories$.next([...this.expenseCategories, category])
				)
			);
	}
}
