...
export class ActivepiecesService {
	...
	getIntegrationStatus(integrationId: ID): Observable<{ enabled: boolean }> {
		return this.http.get<{ enabled: boolean }>(`${API_PREFIX}/integration/activepieces/status/${integrationId}`);
	}
	...
	deleteMcpServer(integrationId: ID, serverId: string): Observable<void> {
		return this.http.delete<void>(`${API_PREFIX}/integration/activepieces/mcp-server/${integrationId}/${serverId}`);
	}
}
