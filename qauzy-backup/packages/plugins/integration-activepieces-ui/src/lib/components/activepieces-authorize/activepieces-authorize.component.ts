...
export class ActivepiecesAuthorizeComponent extends TranslationBaseComponent implements OnInit, OnDestroy {
	...
	setupAndAuthorize() {
		if (this.form.invalid || !this.organization) {
			this._toastrService.error(
				this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.AUTHORIZE.ERRORS.INVALID_FORM')
			);
			return;
		}
			...
			.pipe(
				tap(() => {
					this._toastrService.success(
						this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.AUTHORIZE.SUCCESS.SETTINGS_SAVED')
					);
	...
	startAuthorization() {
		if (!this.organization) {
			this._toastrService.error(
				this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.AUTHORIZE.ERRORS.ORGANIZATION_NOT_FOUND')
			);
			return;
		}
		this.loading = true;
