	...
	private _loadConnection() {
		...
		this._activepiecesService
			...
			.pipe(
				...
				catchError((error) => {
					const errorMessage = this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.ERRORS.LOAD_CONNECTION', {
						error: error.message
					});
	...
	loadConnections() {
		...
		this._activepiecesService
			...
			.pipe(
				...
				catchError((error) => {
					const errorMessage = this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.ERRORS.LOAD_CONNECTIONS', {
						error: error.message
					});
					this._toastrService.error(errorMessage);
					this._activepiecesStore.setError(errorMessage);
	...
	deleteConnection(connection: IActivepiecesConnection) {
		...
		if (connection.integrationId !== this.integrationId) {
			this._toastrService.error(
				this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.ERRORS.INTEGRATION_MISMATCH')
			);
			return;
		}
			...
			.pipe(
				...
				catchError((error) => {
					const errorMessage = this.getTranslation(
						'INTEGRATIONS.ACTIVEPIECES_PAGE.ERRORS.DELETE_CONNECTION',
						{ error: error.message }
					);
					this._toastrService.error(errorMessage);
					this._activepiecesStore.setError(errorMessage);
