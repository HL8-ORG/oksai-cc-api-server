...
export class ActivepiecesCallbackComponent extends TranslationBaseComponent implements OnInit, OnDestroy {
			...
			switchMap((params) => {
				...
				if (!code || !state) {
					this._toastrService.error(
						this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.CALLBACK.ERRORS.INVALID_PARAMETERS')
					);
				...
				if (!this.organization) {
					this._toastrService.error(
						this.getTranslation('ACTIVEPIECES_PAGE.CALLBACK.ERRORS.ORGANIZATION_NOT_FOUND')
					);
					this._redirectToIntegrations();
					return EMPTY;
				}
				...
				return this._activepiecesService.exchangeToken({ code, state }).pipe(
					...
					catchError((error) => {
						const errorMessage =
							this.getTranslation('ACTIVEPIECES_PAGE.CALLBACK.ERRORS.EXCHANGE_TOKEN') +
							': ' +
							error.message;
	...
	createConnection() {
		if (this.form.invalid || !this.organization) {
			this._toastrService.error(
				this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.CALLBACK.ERRORS.INVALID_PROJECT_ID')
			);
			return;
		}
		...
		if (!accessToken) {
			this._toastrService.error(
				this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.CALLBACK.ERRORS.TOKEN_NOT_FOUND')
			);
			this._redirectToIntegrations();
			return;
		}
			...
			.pipe(
				tap((connection: IActivepiecesConnection) => {
					this._toastrService.success(
						this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.CALLBACK.SUCCESS.CONNECTION_CREATED')
					);
				...
				catchError((error) => {
					const errorMessage =
						this.getTranslation('INTEGRATIONS.ACTIVEPIECES_PAGE.CALLBACK.ERRORS.CREATE_CONNECTION') +
						': ' +
						error.message;
					this._toastrService.error(errorMessage);
					this.loading = false;
