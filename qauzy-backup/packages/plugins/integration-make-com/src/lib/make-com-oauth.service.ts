...
import { IntegrationEnum, IIntegrationEntitySetting, IntegrationEntity } from '@gauzy/contracts';
import {
	IntegrationService,
...
export class MakeComOAuthService {
	...
	async exchangeCodeForToken(code: string, state: string): Promise<IMakeComOAuthTokens> {
		try {
			...
			const tokenResponse = await firstValueFrom(
				this.httpService
					.post(`${MAKE_BASE_URL}/oauth/token`, tokenRequestParams, { headers, timeout: 10000 })
					.pipe(
	 ...
	 */
	private async saveIntegrationSettings(
		tokenData: IMakeComOAuthTokens,
		tenantId: string,
