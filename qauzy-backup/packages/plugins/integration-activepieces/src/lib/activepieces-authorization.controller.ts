...
import {
	ACTIVEPIECES_OAUTH_AUTHORIZE_URL,
	ACTIVEPIECES_OAUTH_TOKEN_URL,
	ACTIVEPIECES_SCOPES,
	OAUTH_RESPONSE_TYPE,
	OAUTH_GRANT_TYPE
} from './activepieces.config';
...
export class ActivepiecesAuthorizationController {
	...
	@ApiOperation({
		...
		description:
			'Initiates OAuth flow using tenant-specific configuration if available, otherwise falls back to global config'
	})
	@ApiResponse({
		status: 200,
	...
	async exchangeToken(@Body() body: ActivepiecesTokenExchangeDto): Promise<IActivepiecesOAuthTokens> {
		try {
			...
			if (!activepiecesConfig?.clientId || !activepiecesConfig?.clientSecret) {
				throw new HttpException(
					'ActivePieces OAuth credentials are not configured for this tenant',
