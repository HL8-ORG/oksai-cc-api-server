...
export class ActivepiecesController {
	...
	async upsertConnection(@Body() input: CreateActivepiecesIntegrationDto): Promise<IActivepiecesConnection> {
		try {
			...
			this.logger.error('Failed to upsert ActivePieces connection', {
	...
	async listConnections(
		...
		try {
			...
			this.logger.error('Failed to list ActivePieces connections', {
				message: error?.message,
				stack: error?.stack
			});
	...
	async getTenantConnections(
		...
		try {
			...
			this.logger.error('Failed to get tenant ActivePieces connections', {
				message: error?.message,
				stack: error?.stack
			});
	...
	@Permissions(PermissionsEnum.INTEGRATION_VIEW)
	async getIntegrationTenant(@Param('integrationId', UUIDValidationPipe) integrationId: string): Promise<any> {
		try {
			...
			this.logger.error('Failed to get ActivePieces integration', {
				message: error?.message,
				stack: error?.stack
			});
			throw new HttpException('Failed to get ActivePieces integration tenant', HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	async getConfig(): Promise<Partial<IActivepiecesConfig>> {
		try {
			...
			this.logger.error('Failed to get ActivePieces configuration', {
				message: error?.message,
				stack: error?.stack
			});
			throw new HttpException('Failed to get ActivePieces configuration', HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
}
