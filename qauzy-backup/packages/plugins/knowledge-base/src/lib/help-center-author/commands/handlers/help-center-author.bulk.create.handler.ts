...
export class ArticleAuthorsBulkCreateHandler implements ICommandHandler<ArticleAuthorsBulkCreateCommand> {
	constructor(private readonly helpCenterAuthorService: HelpCenterAuthorService) {}

	public async execute(command: ArticleAuthorsBulkCreateCommand): Promise<IHelpCenterAuthor[]> {
		const { input } = command;
