...
const insertRandomHelpCenterAuthor = async (dataSource: DataSource, data: IHelpCenterAuthor[]) => {
	await dataSource.manager.save(data);
};

const operateData = async (dataSource: DataSource, mapEmployeeToArticles, allArticle, employees: IEmployee[]) => {
