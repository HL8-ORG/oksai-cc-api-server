...
export class UpworkReportService {
		...
		try {
			return new Promise((resolve, reject) => {
				api.setAccessToken(config.accessToken, config.accessSecret, () => {
					reports.getByFreelancerFull(providerId, { tq: selectQuery }, (error: any, data: any) =>
	...
	public async getLimitedReportByFreelance(
		...
		try {
			return new Promise((resolve, reject) => {
				api.setAccessToken(config.accessToken, config.accessSecret, () => {
					reports.getByFreelancerLimited(providerId, { tq: selectQuery }, (error: any, data: any) =>
						error ? reject(error) : resolve(data)
					);
				});
			});
	...
	public async getBillingReportByFreelancer(
		...
		try {
			return new Promise((resolve, reject) => {
				api.setAccessToken(config.accessToken, config.accessSecret, () => {
					billings.getByFreelancer(providerReferenceId, { tq: query }, (error: any, data: any) =>
						error ? reject(error) : resolve(data)
					);
				});
			});
	...
	public async getEarningReportByFreelancer(
		...
		try {
			return new Promise((resolve, reject) => {
				api.setAccessToken(config.accessToken, config.accessSecret, () => {
					earnings.getByFreelancer(providerReferenceId, { tq: query }, (error: any, data: any) =>
						error ? reject(error) : resolve(data)
					);
				});
			});
