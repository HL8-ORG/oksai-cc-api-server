...
export class ErrorReport implements IErrorReport {
	...
	constructor(repository?: IErrorReportRepository, title?: string, description?: string) {
		this.title = title || `Automatic error report for ${this.appName} App ${app.getVersion()}`;
		this.description = description;
		this.repository = repository;
	}
	public async submit(): Promise<void> {
		await shell.openExternal(
			`https://github.com/${this.repository.owner}/${this.repository.name}/issues/new?title=${
				this.title
