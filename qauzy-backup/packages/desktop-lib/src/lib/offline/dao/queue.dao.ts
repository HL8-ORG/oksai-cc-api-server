...
export class AuditQueueDAO implements DAO<AuditQueueTO> {
	...
	public async findOneById(id: number): Promise<AuditQueueTO | undefined> {
		const result = await this._provider.connection<AuditQueueTO>(TABLE_NAME_AUDIT_QUEUE).where('id', '=', id);
		return result[0];
	}
	...
	public async delete(value?: Partial<AuditQueueTO>): Promise<void> {
		...
		await this._provider.connection<AuditQueueTO>(TABLE_NAME_AUDIT_QUEUE).where('id', '=', value.id).del();
	}
