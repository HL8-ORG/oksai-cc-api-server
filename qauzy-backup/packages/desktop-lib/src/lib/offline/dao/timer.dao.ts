...
import { DAO, ITimerTransaction, IDatabaseProvider, ISequence } from '../../interfaces';
import { TABLE_NAME_TIMERS, TABLE_NAME_INTERVALS, TimerTO, UserTO, IntervalTO } from '../dto';
...
export class TimerDAO implements DAO<TimerTO> {
	...
	public async findAllNoSynced(user: UserTO): Promise<ISequence[]> {
		const timers = await this._provider
			.connection<TimerTO>(TABLE_NAME_TIMERS)
			.select(`${TABLE_NAME_TIMERS}.*`, `${TABLE_NAME_INTERVALS}.id as intervalId`)
			.join(TABLE_NAME_INTERVALS, `${TABLE_NAME_TIMERS}.id`, `${TABLE_NAME_INTERVALS}.timerId`)
			...
			.andWhere((qb) =>
				qb.where(`${TABLE_NAME_TIMERS}.synced`, false).orWhereNull(`${TABLE_NAME_TIMERS}.timelogId`)
			)
		...
		const uniqueTimers = [
			...new Map(this._removeColumn('intervalId', timers).map((timer) => [timer.id, timer])).values()
		];
		...
		const timersWithIntervals = uniqueTimers.map((timer: TimerTO) => ({
			...
			intervals: intervals
				...
				.filter((interval: IntervalTO) => interval.timerId === timer.id)
		}));

		return timersWithIntervals.length > 0 ? timersWithIntervals : await this.findFailedRuns(user);
	}
	...
	public async todayDuration(user: UserTO): Promise<Array<{ today_duration_seconds: string | number }>> {
		return await this._provider
			...
			.sum({
				today_duration_seconds: this._provider.connection.client.raw('(stoppedAt/1000 - startedAt/1000)')
			});
	}
}
