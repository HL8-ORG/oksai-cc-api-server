...
export class ScreenshotDAO implements DAO<ScreenshotTO> {
	...
	public async findOneById(id: number): Promise<ScreenshotTO> {
		const result = await this._provider.connection<ScreenshotTO>(TABLE_NAME_SCREENSHOT).where('id', '=', id);
		return result[0];
	}
	...
	public async findUnSyncedScreenshot(): Promise<ScreenshotTO[]> {
		const screenshots = await this._provider.connection<ScreenshotTO>(TABLE_NAME_SCREENSHOT).where('synced', false);
		return screenshots;
	}
}
