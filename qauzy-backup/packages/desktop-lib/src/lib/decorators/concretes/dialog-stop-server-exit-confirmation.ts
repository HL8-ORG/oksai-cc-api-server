...
export class DialogStopServerExitConfirmation extends BaseDesktopDialogDecorator implements IDesktopDialog {
	constructor(dialog: IDesktopDialog) {
		super(new DialogStopTimerExitConfirmation(dialog));
		this.options = {
			...this.options,
			detail: TranslateService.instant('TIMER_TRACKER.DIALOG.EXIT_SERVER_CONFIRM')
		};
	}
}
