...
function getCSPHeader(scriptNonce?: string, styleNonce?: string, baseUrl?: string): string {
	...
	const styleSrc = styleNonce ? `style-src 'self' 'nonce-${styleNonce}'` : "style-src 'self'";

	// Build form-action with explicit origin support
	const formAction = baseUrlOrigin ? `form-action 'self' ${baseUrlOrigin}` : "form-action 'self'";
 ...
 * @param options - Options for customizing security headers
 */
export function getEnhancedSecurityHeaders(options?: {
	scriptNonce?: string;
	styleNonce?: string;
	baseUrl?: string;
}): Record<string, string> {
	...
	const rules = new Map<string, (args: any) => void>([
		[
			...
			(a) => {
				...
				if (!a.password || a.password.length < minPasswordLength)
					errors.push(`Invalid password (minimum length: ${minPasswordLength})`);
			}
		],
		[
			'get_projects',
