	...
	mcp: {
		transport: {
			type: (() => {
				...
				return t === 'stdio' || t === 'http' || t === 'websocket' || t === 'auto'
					? (t as McpTransportType)
					: 'stdio';
			})(),
			http: {
				port: (() => {
				...
				cors: {
					origin: (() => {
						if (process.env.MCP_CORS_ORIGIN) {
							return process.env.MCP_CORS_ORIGIN.split(',')
								.map((o) => o.trim())
								.filter((o) => o.length > 0);
						}
				...
				trustedProxies: process.env.MCP_TRUSTED_PROXIES
					? process.env.MCP_TRUSTED_PROXIES.split(',')
							.map((p) => p.trim())
							.filter((p) => p.length > 0)
					: []
			},
			websocket: {
				...
				allowedOrigins: (() => {
					if (process.env.MCP_WS_ALLOWED_ORIGINS) {
						return process.env.MCP_WS_ALLOWED_ORIGINS.split(',')
							.map((o) => o.trim())
							.filter((o) => o.length > 0);
					}
				...
				trustedProxies: process.env.MCP_WS_TRUSTED_PROXIES
					? process.env.MCP_WS_TRUSTED_PROXIES.split(',')
							.map((p) => p.trim())
							.filter((p) => p.length > 0)
					: []
			}
