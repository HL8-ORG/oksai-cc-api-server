...
export class SessionManager {
	 ...
	 */
	public updateSessionActivity(sessionId: string, metadata?: Record<string, any>, extendTTL?: number): boolean {
	...
	public async checkUserAuthorization(
		...
		if (requiredPermissions.length > 0 && session.permissions) {
			...
			const hasPermissions = requiredPermissions.every((permission) => sessionPermSet.has(permission));
			if (!hasPermissions) {
				return {
					authorized: false,
