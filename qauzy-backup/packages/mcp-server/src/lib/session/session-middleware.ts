	...
	createSessionMiddleware(config: Partial<SessionMiddlewareConfig> = {}) {
		...
		return async (req: Request, res: Response, next: NextFunction) => {
			try {
				...
				// Validate session
				const ipAddress = finalConfig.validateIP
					? this.getClientIP(req, finalConfig.trustedProxies)
					: undefined;
				...
				if (validation.session) {
					sessionManager.addConnection(sessionId, req.connectionId, 'http', {
	...
	createWebSocketSessionMiddleware(config: Partial<SessionMiddlewareConfig> = {}) {
		...
		return async (
			...
			try {
				...
				// Add connection to session
				if (validation.session) {
					const success = sessionManager.addConnection(sessionId, connectionId, 'websocket', {
	...
	createRateLimitMiddleware(
		...
		return rateLimit({
			...
			handler: (req: Request, res: Response) => {
				logger.warn(
					`Rate limit exceeded for session: ${req.sessionId || 'unknown'} from IP: ${this.getClientIP(
						req,
	...
	private isPathAllowed(path: string, allowedPaths: string[]): boolean {
		return allowedPaths.some((allowedPath) => {
			if (allowedPath.includes('*')) {
				// Simple glob pattern matching
				// Escape special regex characters except *
				const escaped = allowedPath.replace(/[.+?^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*');
