...
class SessionMonitor extends EventEmitter {
	...
	performMonitoringCheck(): {
		...
		logger.debug(
			`Monitoring check completed: ${stats.totalSessions} sessions checked, ${alertsGenerated} alerts generated`
		);
	...
	private checkForAlerts(event: SessionEvent): void {
		...
		if (event.type === 'session_created') {
			...
			const timeout = setTimeout(() => {
				this.logAlert({
					level: 'info',
					message: `Long-running session detected: ${event.sessionId} (${
						this.config.alertThresholds.sessionDurationWarning / 1000 / 60
					} minutes)`,
					timestamp: new Date(),
