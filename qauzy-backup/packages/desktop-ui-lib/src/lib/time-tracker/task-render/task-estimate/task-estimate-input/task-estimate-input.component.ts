...
export class TaskEstimateInputComponent implements OnInit, AfterViewInit {
	...
	public ngOnInit() {
		this.timeForm = this.formBuilder.group({
			hourTen: ['', [Validators.required, Validators.min(0), Validators.max(9)]],
			hourUnit: ['', [Validators.required, Validators.min(0), Validators.max(9)]],
			minuteTen: ['', [Validators.required, Validators.min(0), Validators.max(5)]],
			minuteUnit: ['', [Validators.required, Validators.min(0), Validators.max(9)]]
		});
	}
