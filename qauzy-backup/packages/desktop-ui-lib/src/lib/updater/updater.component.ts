import { Component, OnInit, ChangeDetectionStrategy, ViewChild, ElementRef, NgZone, OnDestroy } from '@angular/core';
	...
	handleIpcEvent(_: Event, arg: { type: string; data: any }) {
		switch (arg.type) {
			...
			case 'download_on_progress': {
				this._ngZone.run(() => {
					this.notAvailable = true;
					this.message = `Update Downloading ${arg.data.percent ? Math.floor(Number(arg.data.percent)) : 0}%`;
					this.logContents.push(this.message);
					this.scrollToBottom();
				});
				break;
