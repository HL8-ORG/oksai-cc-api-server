	...
	public load(iconUrl: string): Observable<SafeUrl> {
		...
		if (!icon$) {
			icon$ = this._http.get(iconUrl, { responseType: 'blob' }).pipe(
				concatMap((response: Blob) => this._imageService.getBase64ImageFromBlob(response)),
				shareReplay(1)
			);
			this._statusIconCache.setValue(icon$, iconUrl);
		}
		return icon$.pipe(map((base64Image: string) => this._domSanitizer.bypassSecurityTrustUrl(base64Image)));
	}
}
