...
export class ImageCacheService extends AbstractCacheService<string> {
	...
	constructor(protected _storageService: StorageService<string>, protected _store: Store) {
	...
	public clear() {
		if (moment(new Date()).diff(this._lastCached, 'milliseconds') > this.duration) {
			super.clear();
			this._lastCached = new Date();
		}
	}
	...
	public setValue(value: Observable<string>, object: string) {
		if (this._size > this._limit) {
			...
			imageCache ? this._storageService.removeItem(this._key(imageCache.data.object)) : super.clear();
		}
	...
	private get _size(): number {
		return this._storageService.getAllKeys().filter((key: string) => key.includes(this.prefix)).length;
	}
