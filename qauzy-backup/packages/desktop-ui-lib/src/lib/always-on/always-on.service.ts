...
export class AlwaysOnService {
	...
	public get state$(): Observable<AlwaysOnStateEnum> {
		return new Observable((observer) => {
			this._electronService.ipcRenderer.on('change_state_from_ao', (_, state: AlwaysOnStateEnum) => {
				observer.next(state);
			});
		});
	}
	...
	public get counter$(): Observable<ITimeCounter> {
		return new Observable((observer) => {
			this._electronService.ipcRenderer.on('ao_time_update', (_, count: ITimeCounter) => {
				observer.next(count);
			});
		});
	}
	...
	public get checkTimerStatus$(): Observable<number> {
		return new Observable((observer) => {
			this._electronService.ipcRenderer.on('check_timer_status', (_, arg: number) => {
				observer.next(arg);
			});
		});
	}
