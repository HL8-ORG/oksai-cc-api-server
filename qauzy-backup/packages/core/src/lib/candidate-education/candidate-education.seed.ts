...
export const createCandidateEducations = async (
	...
	for (const candidate of candidates) {
		...
		defaultCandidateEducation = [...defaultCandidateEducation, ...educations];
	}
...
export const createRandomCandidateEducations = async (
	...
	for await (const tenant of tenants || []) {
		...
		for (const candidate of candidates) {
			...
			candidateEducation = [...DEFAULT_CANDIDATE_EDUCATIONS, ...educations];
		}
	}
...
const insertCandidateEducations = async (dataSource: DataSource, educations: CandidateEducation[]) => {
	await dataSource.createQueryBuilder().insert().into(CandidateEducation).values(educations).execute();
};
