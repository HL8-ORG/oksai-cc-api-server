...
export class ActivityUpdateHandler implements ICommandHandler<ActivityUpdateCommand> {
	...
	public async execute(command: ActivityUpdateCommand): Promise<any> {
		try {
			...
			throw new BadRequestException(
				error,
				`Can'\t update ${IntegrationEntity.ACTIVITY} for ${IntegrationEntity.TIME_SLOT}`
			);
		}
	}
}
