...
export class ActivityCreateHandler implements ICommandHandler<ActivityCreateCommand> {
	constructor(
	...
	public async execute(command: ActivityCreateCommand): Promise<any> {
		try {
			...
			throw new BadRequestException(
				error,
				`Can'\t create ${IntegrationEntity.ACTIVITY} for ${IntegrationEntity.TIME_SLOT}`
			);
		}
	}
}
