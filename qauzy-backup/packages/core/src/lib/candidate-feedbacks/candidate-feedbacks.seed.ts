...
const insertCandidateFeedbacks = async (dataSource: DataSource, candidateFeedbacks: CandidateFeedback[]) => {
	await dataSource.createQueryBuilder().insert().into(CandidateFeedback).values(candidateFeedbacks).execute();
};
