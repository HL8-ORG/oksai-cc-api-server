...
export class KeyResultBulkCreateHandler implements ICommandHandler<KeyResultBulkCreateCommand> {
	constructor(private readonly keyResultService: KeyResultService) {}

	public async execute(command: KeyResultBulkCreateCommand): Promise<IKeyResult[]> {
		const { input } = command;
