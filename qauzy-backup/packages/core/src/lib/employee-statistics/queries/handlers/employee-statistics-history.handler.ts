...
export class EmployeeStatisticsHistoryQueryHandler implements IQueryHandler<EmployeeStatisticsHistoryQuery> {
	constructor(private employeeStatisticsService: EmployeeStatisticsService) {}

	public async execute(command: EmployeeStatisticsHistoryQuery): Promise<IEmployeeStatisticsHistory[]> {
		const { input } = command;
	...
	private async _incomeHistory(input: IEmployeeStatisticsHistoryFindInput): Promise<IEmployeeStatisticsHistory[]> {
		const {
	...
	private async _expenseHistory(input: IEmployeeStatisticsHistoryFindInput): Promise<IEmployeeStatisticsHistory[]> {
		const history: IEmployeeStatisticsHistory[] = [];
		...
		return input.type === EmployeeStatisticsHistoryEnum.EXPENSES_WITHOUT_SALARY
		...
		employeeRecurringExpenses.forEach((expense) => {
			...
			for (
				...
				history.push({
					...
					isSalary: expense.categoryName === RecurringExpenseDefaultCategoriesEnum.SALARY
				});
			}
		});
	}
