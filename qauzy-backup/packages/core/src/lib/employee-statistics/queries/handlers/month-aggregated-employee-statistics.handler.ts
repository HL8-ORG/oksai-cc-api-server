	...
	implements IQueryHandler<MonthAggregatedEmployeeStatisticsQuery>
{
	...
	public async execute(
		 ...
		 */
		const statisticsMap: Map<string, IMonthAggregatedEmployeeStatistics> = new Map();
		...
		incomes.forEach((income) => {
			...
			if (statisticsMap.has(key)) {
				...
				const newStat: IMonthAggregatedEmployeeStatistics = {
					...
					profit: 0,
					directIncomeBonus: income.isBonus ? Number(amount.toFixed(2)) : 0,
	...
	private async _loadEmployeeRecurringExpenses(
		...
		employeeRecurringExpenses.forEach((expense) => {
			...
			for (
				...
				const salaryExpense = expense.categoryName === RecurringExpenseDefaultCategoriesEnum.SALARY;
				...
				if (statisticsMap.has(key)) {
					...
					const newStat: IMonthAggregatedEmployeeStatistics = {
						...
						income: 0,
						expense: Number(amount.toFixed(2)),
						expenseWithoutSalary: salaryExpense ? 0 : Number(amount.toFixed(2)),
						profit: 0,
						bonus: 0,
						directIncomeBonus: 0
					};
	...
	private _calculateProfit(statisticsMap: Map<string, IMonthAggregatedEmployeeStatistics>) {
		// For every stat entry in the map, update profit value
