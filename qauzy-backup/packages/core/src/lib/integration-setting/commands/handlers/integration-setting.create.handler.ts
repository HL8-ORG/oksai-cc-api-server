...
export class IntegrationSettingCreateHandler implements ICommandHandler<IntegrationSettingCreateCommand> {
	constructor(private readonly integrationSettingService: IntegrationSettingService) {}

	public async execute(command: IntegrationSettingCreateCommand): Promise<IIntegrationSetting> {
		const { input } = command;
