...
export class OrganizationTaskSettingCreateHandler implements ICommandHandler<OrganizationTaskSettingCreateCommand> {
	constructor(private readonly _organizationTaskSettingService: OrganizationTaskSettingService) {}
	 ...
	 */
	public async execute(command: OrganizationTaskSettingCreateCommand): Promise<IOrganizationTaskSetting> {
		try {
			const { input } = command;
