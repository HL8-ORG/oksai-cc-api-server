...
export class AvailabilitySlotsBulkCreateHandler implements ICommandHandler<AvailabilitySlotsBulkCreateCommand> {
	constructor(private readonly commandBus: CommandBus) {}

	public async execute(command: AvailabilitySlotsBulkCreateCommand): Promise<IAvailabilitySlot[]> {
		const { input } = command;
		...
		for (const item of input) {
			...
			availabilitySlots = await this.commandBus.execute(new AvailabilitySlotsCreateCommand(availabilitySlots));
