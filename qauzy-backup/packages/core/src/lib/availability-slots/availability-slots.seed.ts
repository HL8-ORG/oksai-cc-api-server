...
const dataOperation = async (
	...
	for (let i = 0; i < noOfAvailabilitySlotsPerOrganization; i++) {
		...
		slot.employee = faker.helpers.arrayElement([faker.helpers.arrayElement(employees), null]);
		...
		slot.type = faker.helpers.arrayElement(Object.values(AvailabilitySlotType));
		slots.push(slot);
	}
