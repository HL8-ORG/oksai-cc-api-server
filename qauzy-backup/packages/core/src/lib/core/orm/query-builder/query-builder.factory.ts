...
export function createQueryBuilder<Entity>(
	repository: Repository<Entity> | EntityRepository<any>,
	alias?: string
): IQueryBuilder<Entity> {
 ...
 */
export function multiORMCreateQueryBuilder<Entity>(
	repository: CombinedEntityRepository<Entity>,
	ormType: MultiORMEnum = MultiORMEnum.TypeORM,
	alias?: string
): IQueryBuilder<Entity> {
	switch (ormType) {
