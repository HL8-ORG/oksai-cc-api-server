...
export class MikroOrmQueryBuilder<Entity extends object> implements IQueryBuilder<Entity> {
	...
	constructor(private readonly repository: EntityRepository<Entity>, public readonly queryAlias?: string) {
	...
	from(
		entityTarget: ((qb: QueryBuilder<any>) => QueryBuilder<any>) | EntityTarget<Entity>,
		aliasName?: string
	): this {
	...
	addFrom(
		_entityTarget: ((qb: QueryBuilder<any>) => QueryBuilder<any>) | EntityTarget<Entity>,
		_aliasName?: string
	): this {
		throw new Error(
			`Note: This is conceptual; MikrORM typically don't support multiple FROMs like typeORM. You might use this for sub-queries or additional joins instead.`
		);
	}
	...
	where(
		condition: string | object | ((qb: IQueryBuilder<Entity>) => IQueryBuilder<Entity>),
		parameters?: Record<string, any>
	): this {
		if (typeof condition === 'string') {
			const [mikroOrmCondition, mikroOrmParameters] = convertTypeOrmConationAndParamsToMikroOrm(
				condition,
				parameters
			);
	...
	private applyRelationsToQueryBuilder<Entity>(qb: QueryBuilder<any>, relations: string[]) {
