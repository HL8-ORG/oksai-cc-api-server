...
export class ImportRecordUpdateOrCreateHandler implements ICommandHandler<ImportRecordUpdateOrCreateCommand> {
	constructor(
		@Inject(forwardRef(() => ImportRecordService))
		private readonly _importRecordService: ImportRecordService
	) {}

	public async execute(event: ImportRecordUpdateOrCreateCommand): Promise<IImportRecord> {
		const { options, input = {} as IImportRecord } = event;
		const payload = Object.assign({}, options, input) as IImportRecord;

		const { sourceId, destinationId, entityType, tenantId = RequestContext.currentTenantId() } = payload;
