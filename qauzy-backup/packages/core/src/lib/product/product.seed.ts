...
export const createDefaultProducts = async (dataSource: DataSource, tenant: ITenant, organization: IOrganization) => {
	...
	for (let i = 0; i <= 30; i++) {
		...
		product.productCategory = productCategories[Math.floor(Math.random() * productCategories.length)];
...
const insertProduct = async (dataSource: DataSource, products: Product[]): Promise<void> => {
