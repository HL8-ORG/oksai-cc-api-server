...
const insertDefaultPolicy = async (dataSource: DataSource, defaultPolicy: ApprovalPolicy): Promise<ApprovalPolicy> => {
	await dataSource.createQueryBuilder().insert().into(ApprovalPolicy).values(defaultPolicy).execute();
...
export const createRandomApprovalPolicyForOrg = async (
	...
	await dataSource.createQueryBuilder().insert().into(ApprovalPolicy).values(policies).execute();
