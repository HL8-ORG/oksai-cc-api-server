...
export class RequestApprovalPolicyGetHandler implements ICommandHandler<RequestApprovalPolicyGetCommand> {
	constructor(private readonly approvalPolicyService: ApprovalPolicyService) {}

	public async execute(command: RequestApprovalPolicyGetCommand): Promise<IPagination<IApprovalPolicy>> {
		const { input } = command;
