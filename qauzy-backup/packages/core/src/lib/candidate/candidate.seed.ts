...
export const createRandomCandidates = async (
	...
	for await (const tenant of tenants) {
		...
		const insertCandidatesInToOrganization = async (quantity: number, organization: IOrganization) => {
...
const insertCandidates = async (dataSource: DataSource, candidates: Candidate[]): Promise<Candidate[]> => {
