...
export class ReportOrganizationCreateHandler implements ICommandHandler<ReportOrganizationCreateCommand> {
	constructor(private readonly _reportOrganizationService: ReportOrganizationService) {}
	...
	public async execute(event: ReportOrganizationCreateCommand) {
		try {
			...
			console.error(
				`Error occurred while executing bulk creation of report organization entries: ${error.message}`
			);
		}
	}
}
