...
export const createCountries = async (dataSource: DataSource): Promise<ICountry[]> => {
...
const insertCountry = async (dataSource: DataSource, countries: ICountry[]): Promise<void> => {
	await dataSource.createQueryBuilder().insert().into(Country).values(countries).execute();
};
