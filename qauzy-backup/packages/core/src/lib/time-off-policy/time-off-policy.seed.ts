...
const insertDefaultPolicy = async (dataSource: DataSource, defaultPolicy: TimeOffPolicy): Promise<void> => {
	await dataSource.createQueryBuilder().insert().into(TimeOffPolicy).values(defaultPolicy).execute();
};
	...
	(tenants || []).forEach((tenant) => {
		...
		(organizations || []).forEach((organization) => {
			DEFAULT_TIMEOFF_POLICIES.forEach((name) => {
				...
				policy.requiresApproval = faker.helpers.arrayElement([true, false]);
