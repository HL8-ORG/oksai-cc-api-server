 ...
 */
export const createDefaultAccountingTemplates = async (dataSource: DataSource): Promise<any> => {
	try {
		const templatePath = ['core', 'seeds', 'data', 'default-accounting-templates'];
		const files = [];
...
const insertTemplate = async (dataSource: DataSource, accountingTemplate: AccountingTemplate): Promise<void> => {
	await dataSource.createQueryBuilder().insert().into(AccountingTemplate).values(accountingTemplate).execute();
};

const pathToAccountingTemplate = async (fullPath: string): Promise<AccountingTemplate> => {
	try {
