...
export class ExpenseCategoryFirstOrCreateHandler implements ICommandHandler<ExpenseCategoryFirstOrCreateCommand> {
	constructor(
		private readonly _expenseCategoryService: ExpenseCategoriesService,
		private readonly _commandBus: CommandBus
	) {}

	public async execute(command: ExpenseCategoryFirstOrCreateCommand) {
		const { input } = command;
		try {
			...
			return await this._commandBus.execute(new ExpenseCategoryCreateCommand(input));
		}
	}
}
