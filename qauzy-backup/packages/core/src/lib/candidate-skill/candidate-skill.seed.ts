...
const insertCandidateSkills = async (dataSource: DataSource, candidateSkills: CandidateSkill[]) => {
	await dataSource.createQueryBuilder().insert().into(CandidateSkill).values(candidateSkills).execute();
};
