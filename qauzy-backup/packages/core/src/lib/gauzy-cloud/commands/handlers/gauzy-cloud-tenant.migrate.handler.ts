...
export class GauzyCloudTenantMigrateHandler implements ICommandHandler<GauzyCloudTenantMigrateCommand> {
	...
	private async migrateRoles(tenant: ITenant, token: string) {
		return this._gauzyCloudService.migrateRoles(await this._roleService.migrateRoles(), token, tenant).subscribe();
	}

	private async migratePermissions(tenant: ITenant, token: string) {
		return this._gauzyCloudService
			.migrateRolePermissions(await this._rolePermissionService.migratePermissions(), token, tenant)
			.subscribe();
	}
}
