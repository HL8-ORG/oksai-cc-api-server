...
import { IOrganizationContact, ContactOrganizationInviteStatus, RolesEnum, ID } from '@gauzy/contracts';
...
export class InviteOrganizationContactHandler implements ICommandHandler<InviteOrganizationContactCommand> {
	constructor(
	 ...
	 */
	public async execute(command: InviteOrganizationContactCommand): Promise<IOrganizationContact> {
		try {
			const {
			...
			if (alreadyExists) {
				throw new InternalServerErrorException('Contact email already exists in the account as a user');
			}
