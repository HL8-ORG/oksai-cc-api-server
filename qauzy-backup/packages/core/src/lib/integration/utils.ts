		...
		for await (const {
			...
			try {
				...
				switch (queryRunner.connection.options.type) {
					...
					default:
						throw Error(
							`cannot upsert integration and integration types due to unsupported database type: ${queryRunner.connection.options.type}`
						);
				}
	...
	public static async upsertIntegrationTypes(queryRunner: QueryRunner, integrationTypeNames: IntegrationTypeEnum[]) {
		for await (const integrationTypeName of integrationTypeNames) {
			...
			switch (queryRunner.connection.options.type) {
				...
				default:
					throw Error(
						`cannot upsert integration types due to unsupported database type: ${queryRunner.connection.options.type}`
					);
			}
		...
		if (integration) {
			...
			for await (const integrationType of integrationTypes) {
				...
				switch (queryRunner.connection.options.type) {
					...
					default:
						throw Error(
							`cannot sync integration types due to unsupported database type: ${queryRunner.connection.options.type}`
						);
				}
