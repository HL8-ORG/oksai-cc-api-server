...
export class CreateIntegrationOrganizationGithubRepositoryIssueTable1698383136452 implements MigrationInterface {
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`CREATE TABLE "organization_github_repository_issue" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "isActive" boolean DEFAULT true, "isArchived" boolean DEFAULT false, "tenantId" uuid, "organizationId" uuid, "issueId" integer NOT NULL, "issueNumber" integer NOT NULL, "repositoryId" uuid, CONSTRAINT "PK_99e44380c05f06ffe7c9200f9c5" PRIMARY KEY ("id"))`
		);
		await queryRunner.query(
	...
	public async postgresDownQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`ALTER TABLE "organization_github_repository_issue" DROP CONSTRAINT "FK_5065401113abb6e9608225e5678"`
		);
		await queryRunner.query(
			`ALTER TABLE "organization_github_repository_issue" DROP CONSTRAINT "FK_6c8e119fc6a2a7d3413aa76d3bd"`
		);
		await queryRunner.query(
			`ALTER TABLE "organization_github_repository_issue" DROP CONSTRAINT "FK_b3234be5b70c2362cdf67bb1889"`
		);
	...
	public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`CREATE TABLE "organization_github_repository_issue" ("id" varchar PRIMARY KEY NOT NULL, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "isActive" boolean DEFAULT (1), "isArchived" boolean DEFAULT (0), "tenantId" varchar, "organizationId" varchar, "issueId" integer NOT NULL, "issueNumber" integer NOT NULL, "repositoryId" varchar)`
		);
		await queryRunner.query(
			`CREATE INDEX "IDX_d706210d377ece2a1bc3386388" ON "organization_github_repository_issue" ("isActive") `
		);
