...
export class AlterCandidateFeedbackTable1664869127437 implements MigrationInterface {
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE TYPE "public"."candidate_feedback_status_enum" AS ENUM('APPLIED', 'REJECTED', 'HIRED')`
		);
		await queryRunner.query(
			`ALTER TABLE "candidate_feedback" ADD "status" "public"."candidate_feedback_status_enum"`
		);
	}
