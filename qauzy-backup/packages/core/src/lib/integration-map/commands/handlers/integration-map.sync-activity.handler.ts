...
export class IntegrationMapSyncActivityHandler implements ICommandHandler<IntegrationMapSyncActivityCommand> {
	constructor(
	 ...
	 * @returns
	 */
	public async execute(command: IntegrationMapSyncActivityCommand) {
		const { input } = command;
		...
		try {
			...
			const gauzyActivity = await this._commandBus.execute(new ActivityCreateCommand(entity));
