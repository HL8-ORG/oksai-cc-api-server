...
export class IntegrationMapSyncTimeLogHandler implements ICommandHandler<IntegrationMapSyncTimeLogCommand> {
	constructor(
	 ...
	 * @returns
	 */
	public async execute(command: IntegrationMapSyncTimeLogCommand) {
		const { input } = command;
		...
		try {
			...
			const gauzyTimeLog = await this._commandBus.execute(new TimeLogCreateCommand(entity));
