...
export class IntegrationMapSyncOrganizationHandler implements ICommandHandler<IntegrationMapSyncOrganizationCommand> {
	constructor(
	 ...
	 * @returns
	 */
	public async execute(command: IntegrationMapSyncOrganizationCommand) {
		const { input } = command;
		...
		try {
			...
			await this._commandBus.execute(new OrganizationUpdateCommand(organizationMap.gauzyId, entity));
			return organizationMap;
		} catch (error) {
			const organization = await this._commandBus.execute(new OrganizationCreateCommand(entity));
