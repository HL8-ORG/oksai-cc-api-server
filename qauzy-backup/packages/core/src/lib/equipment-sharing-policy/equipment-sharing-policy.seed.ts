...
const insertDefaultPolicy = async (dataSource: DataSource, defaultPolicy: EquipmentSharingPolicy): Promise<void> => {
	await dataSource.createQueryBuilder().insert().into(EquipmentSharingPolicy).values(defaultPolicy).execute();
};
...
export const createRandomEquipmentSharingPolicy = async (
	...
	await dataSource.createQueryBuilder().insert().into(EquipmentSharingPolicy).values(policies).execute();
	return policies;
};
