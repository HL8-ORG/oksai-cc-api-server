...
export const createCandidateDocuments = async (
	...
	for (const candidate of candidates) {
		...
		defaultCandidateDocuments = [...defaultCandidateDocuments, ...documents];
	}
...
const insertCandidateDocuments = async (dataSource: DataSource, candidateDocuments: CandidateDocument[]) => {
	await dataSource.createQueryBuilder().insert().into(CandidateDocument).values(candidateDocuments).execute();
};
