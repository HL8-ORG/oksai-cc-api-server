...
export class EventTypeCreateHandler implements ICommandHandler<EventTypeCreateCommand> {
	constructor(
	...
	public async execute(command: EventTypeCreateCommand): Promise<EventType> {
		...
		const employee = input.employeeId ? await this.employeeService.findOneByIdString(input.employeeId) : null;
