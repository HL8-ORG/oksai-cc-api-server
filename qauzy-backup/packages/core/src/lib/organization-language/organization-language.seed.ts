...
export const createRandomOrganizationLanguage = async (
	...
	const mapOrganizationLanguage: IOrganizationLanguage[] = [];
	const allLanguage = await dataSource.manager
...
const insertRandomOrganizationLanguage = async (dataSource: DataSource, data: IOrganizationLanguage[]) => {
	await dataSource.createQueryBuilder().insert().into(OrganizationLanguage).values(data).execute();
};
