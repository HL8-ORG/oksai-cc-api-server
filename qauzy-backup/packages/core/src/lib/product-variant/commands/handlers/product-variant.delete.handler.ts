...
export class ProductVariantDeleteHandler implements ICommandHandler<ProductVariantDeleteCommand> {
	constructor(
	...
	public async execute(command?: ProductVariantDeleteCommand): Promise<DeleteResult> {
		...
		return {
			...
			affected: deleteRes.map((res) => (res.affected ? res.affected : 0)).reduce((acc, value) => acc + value)
		};
	}
}
