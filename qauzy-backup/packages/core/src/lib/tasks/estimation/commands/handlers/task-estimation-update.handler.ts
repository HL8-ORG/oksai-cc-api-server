...
export class TaskEstimationUpdateHandler implements ICommandHandler<TaskEstimationUpdateCommand> {
	constructor(
		private readonly _taskEstimationService: TaskEstimationService,
		private readonly commandBus: CommandBus
	) {}

	public async execute(command: TaskEstimationUpdateCommand): Promise<ITaskEstimation> {
	 ...
	 * @returns
	 */
	public async update(id: string, request: ITaskEstimationUpdateInput): Promise<ITaskEstimation> {
		try {
			...
			await this.commandBus.execute(new TaskEstimationCalculateCommand(taskEstimation.taskId));
