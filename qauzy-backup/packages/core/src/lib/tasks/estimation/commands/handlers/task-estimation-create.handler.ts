...
export class TaskEstimationCreateHandler implements ICommandHandler<TaskEstimationCreateCommand> {
	constructor(
		private readonly _taskEstimationService: TaskEstimationService,
		private readonly commandBus: CommandBus
	) {}

	public async execute(command: TaskEstimationCreateCommand): Promise<ITaskEstimation> {
		const { input } = command;
		let taskEstimationAlreadyPresent: TaskEstimation;

		try {
			taskEstimationAlreadyPresent = await this._taskEstimationService.findOneByWhereOptions({
		...
		if (taskEstimationAlreadyPresent) {
			throw new BadRequestException('Record for this member is already present');
		}

		try {
			const taskEstimation = await this._taskEstimationService.create(input);
			await this.commandBus.execute(new TaskEstimationCalculateCommand(input.taskId));
