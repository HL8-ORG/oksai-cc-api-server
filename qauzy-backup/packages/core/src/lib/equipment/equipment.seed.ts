...
export const createDefaultEquipments = async (
	...
	const tags = await dataSource.getRepository(Tag).createQueryBuilder().getMany();
...
const insertEquipment = async (dataSource: DataSource, equipment: Equipment[]): Promise<void> => {
...
export const createRandomEquipments = async (
	...
	for await (const tenant of tenants || []) {
		...
		for (let i = 0; i < noOfEquipmentsPerTenant; i++) {
			const equipment = new Equipment();
			const randomElement = faker.helpers.arrayElement(DEFAULT_RANDOM_EQUIPMENTS);
