...
import { KeyResultUpdateStatusEnum, KeyResultTypeEnum, IOrganization, ITenant } from '@gauzy/contracts';
	...
	for await (const keyResult of keyResults) {
		...
		for (let i = 0; i < numberOfUpdates; i++) {
			...
			if (moment().isAfter(startDate)) {
				...
				keyResultUpdate.status = faker.helpers.arrayElement(Object.values(KeyResultUpdateStatusEnum));
