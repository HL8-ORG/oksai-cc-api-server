...
export class BroadcastController extends CrudController<Broadcast> {
	constructor(private readonly broadcastService: BroadcastService, private readonly commandBus: CommandBus) {
	...
	async findById(
		...
		@Query() params: BaseQueryDTO<Broadcast>
	): Promise<IBroadcast> {
	...
	async update(
		@Param('id', UUIDValidationPipe) id: ID,
		@Body() entity: UpdateBroadcastDTO
	): Promise<IBroadcast | UpdateResult> {
