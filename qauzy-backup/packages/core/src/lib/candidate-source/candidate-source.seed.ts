...
const insertCandidateSources = async (dataSource: DataSource, candidateSources: CandidateSource[]) => {
	await dataSource.createQueryBuilder().insert().into(CandidateSource).values(candidateSources).execute();
};
