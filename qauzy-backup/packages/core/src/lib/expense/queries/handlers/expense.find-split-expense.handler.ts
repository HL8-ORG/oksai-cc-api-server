...
export class FindSplitExpenseHandler implements IQueryHandler<FindSplitExpenseQuery> {
	constructor(private readonly expenseService: ExpenseService, private readonly employeeService: EmployeeService) {}

	public async execute(query: FindSplitExpenseQuery): Promise<IPagination<ISplitExpenseOutput>> {
		const {
		...
		const splitItems = items.map((e) =>
			e.splitExpense
				? {
						...e,
						amount: +(e.amount / (orgEmployees.total !== 0 ? orgEmployees.total : 1)).toFixed(2),
