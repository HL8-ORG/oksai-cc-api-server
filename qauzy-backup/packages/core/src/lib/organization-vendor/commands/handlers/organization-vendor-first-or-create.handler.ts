...
export class OrganizationVendorFirstOrCreateHandler implements ICommandHandler<OrganizationVendorFirstOrCreateCommand> {
	constructor(private readonly _organizationVendorService: OrganizationVendorService) {}

	public async execute(command: OrganizationVendorFirstOrCreateCommand): Promise<IOrganizationVendor> {
		const { input } = command;
		try {
