...
export const createOrganizationVendors = async (
	...
	for (const organization of organizations) {
		...
		defaultOrganizationVendors = [...defaultOrganizationVendors, ...vendors];
	}
...
export const createRandomOrganizationVendors = async (
	...
	const organizationVendorsMap: Map<IOrganization, IOrganizationVendor[]> = new Map();
...
const insertOrganizationVendors = async (dataSource: DataSource, organizationVendors: OrganizationVendor[]) => {
	await dataSource.createQueryBuilder().insert().into(OrganizationVendor).values(organizationVendors).execute();
};
