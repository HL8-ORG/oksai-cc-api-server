...
const insertDefaultTimeFrames = async (dataSource: DataSource, defaultTimeFrames: GoalTimeFrame[]) => {
	await dataSource.createQueryBuilder().insert().into(GoalTimeFrame).values(defaultTimeFrames).execute();
};
