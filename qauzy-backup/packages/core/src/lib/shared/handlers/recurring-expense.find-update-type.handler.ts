 ...
 */
export abstract class FindRecurringExpenseStartDateUpdateTypeHandler<T extends IRecurringExpenseModel> {
	constructor(private readonly crudService: CrudService<T>) {}

	public async executeQuery(input: IFindStartDateUpdateTypeInput): Promise<IStartUpdateTypeInfo> {
		...
		if (oldStartDateObject.getTime() === newStartDateObject.getTime()) {
			...
			newStartDateObject.getFullYear() === oldStartDateObject.getFullYear()
		) {
			return {
	...
	private async getIncreaseType(originalExpense, newStartDateObject) {
		const safeUpdateType =
			originalExpense.endDate && newStartDateObject.getTime() > originalExpense.endDate.getTime()
				? StartDateUpdateTypeEnum.INCREASE_SAFE_OUTSIDE_LIMIT
				: StartDateUpdateTypeEnum.INCREASE_SAFE_WITHIN_LIMIT;

		const { items: foundRecurringExpenses, total } = await this.findAllExpensesInBetween(
			originalExpense.id,
			...
			new Date(
				...
				getLastDayOfMonth(newStartDateObject.getFullYear(), newStartDateObject.getMonth())
			)
		);

		return {
			value: total === 0 ? safeUpdateType : StartDateUpdateTypeEnum.INCREASE_CONFLICT,
	...
	private async getReduceType(originalExpense, newStartDate) {
		const currentStartDate = new Date(
			...
			getLastDayOfMonth(originalExpense.startYear, originalExpense.startMonth)
		);
		...
		return {
			value: total === 0 ? StartDateUpdateTypeEnum.REDUCE_SAFE : StartDateUpdateTypeEnum.REDUCE_CONFLICT,
			conflicts: foundRecurringExpenses
		};
	}
