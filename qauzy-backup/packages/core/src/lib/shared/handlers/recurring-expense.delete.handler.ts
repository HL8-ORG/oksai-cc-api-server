 ...
 */
export abstract class RecurringExpenseDeleteHandler<T extends IRecurringExpenseModel> {
	...
	public async executeCommand(
		...
		switch (deleteInput.deletionType) {
			...
			case RecurringExpenseDeletionEnum.CURRENT:
				...
				break;
			default:
				throw new BadRequestException(`Unsupported deletion type ${deleteInput.deletionType}`);
		}
	 ...
	 */
	private async updateEndDateToLastMonth(id: string, deleteInput: IRecurringExpenseDeleteInput): Promise<any> {
	...
	private async createExpenseFromNextMonth(
		...
		if (!originalExpense.endDate || nextStartDate.getTime() <= originalExpense.endDate.getTime()) {
			const createOptions: any = {
				startDay: 1,
				startMonth: deleteInput.month + 1,
				startYear: deleteInput.year,
				startDate: nextStartDate,
				endDay: getLastDayOfMonth(originalExpense.endYear, originalExpense.endMonth),
