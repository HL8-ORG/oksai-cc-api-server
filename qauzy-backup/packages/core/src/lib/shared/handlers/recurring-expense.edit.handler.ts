...
import { IRecurringExpenseEditInput, IRecurringExpenseModel, StartDateUpdateTypeEnum } from '@gauzy/contracts';
 ...
 */
export abstract class RecurringExpenseEditHandler<T extends IRecurringExpenseModel> {
	constructor(private readonly crudService: CrudService<T>) {}

	public async executeCommand(id: string, input: IRecurringExpenseEditInput): Promise<any> {
		...
		switch (startDateUpdateType) {
			...
			case StartDateUpdateTypeEnum.INCREASE_CONFLICT:
				...
				throw new BadRequestException('Cannot increase start date with conflicts');
			default:
				throw new BadRequestException('Start Date Update Type Not Defined');
		}
	}
	 ...
	 */
	private updateExpenseStartDateAndValue = async (id: string, input: IRecurringExpenseEditInput) => {
		//TODO: Fix typescript
	...
	private increaseSafe = async (
		...
		const createObject: any = {
			...
			endMonth: originalEndDate > newStartDate ? originalExpense.endMonth : null,
	 ...
	 */
	private async reduceEndDateToPreviousMonth(id: string, startYear: number, startMonth: number) {
	...
	async findAllExpensesInBetween(
		...
		const lastDayOfMonth = getLastDayOfMonth(currentStartYear, currentStartMonth);
