...
export class OAuth2ClientManager {
	...
	private initializeDefaultClients() {
		...
		void this.registerClient(
			...
			'test-client'
		).catch((err) => this.securityLogger.error('Default client init failed:', err as Error));
		...
		void this.registerClient(
			{
				...
				client_type: 'public',
				redirect_uris: ['https://chatgpt.com/oauth/callback', 'https://chat.openai.com/oauth/callback'],
				grant_types: ['authorization_code', 'refresh_token'],
				response_types: ['code'],
				scope: 'mcp.read mcp.write',
		...
		void this.registerClient(
			{
				...
				client_type: 'public',
				redirect_uris: ['http://localhost:*', 'https://claude.ai/oauth/callback'],
