...
export class OAuth2AuthorizationCodeManager {
	...
	exchangeAuthorizationCode(
		...
		if (authCode.clientId !== clientId) {
			this.securityLogger.warn(
				`Client ID mismatch for code ${this.redactAuthorizationCode(code)}: expected ${
					authCode.clientId
				}, got ${clientId}`
			);
			return null;
		...
		this.securityLogger.log(
			`Authorization code exchanged successfully for user ${authCode.userId}, client ${clientId}`
		);
	 ...
	 */
	private validatePKCE(codeChallenge: string, method: 'S256' | 'plain' | undefined, codeVerifier: string): boolean {
		const m = method ?? 'S256';
		if (m === 'plain') {
