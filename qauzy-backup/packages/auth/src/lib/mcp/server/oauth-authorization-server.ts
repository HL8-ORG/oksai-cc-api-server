...
export class OAuth2AuthorizationServer {
	...
	constructor(private config: OAuth2ServerConfig) {
		...
		const authConfig: AuthorizationConfig = {
			...
			jwt: {
				...
				issuer: config.issuer,
				algorithms: (this.tokenManager
					.getJWKS()
					.keys?.map((k: any) => k.alg)
					.filter(Boolean) as string[]) || ['RS256'],
	...
	private initializeCSRFProtection() {
		...
		const csrfCookieName = isHttps
			? cookieDomain
				? '__Secure-mcp.x-csrf-token'
				: '__Host-mcp.x-csrf-token'
			: 'mcp-csrf-token';
	...
	private validateConfiguration(): void {
		...
		if (!this.authenticateUser) {
			errors.push(
				'User authenticator is not configured. Call setUserAuthenticator() to provide authentication logic.'
			);
		}

		if (!this.userInfoProvider) {
			errors.push(
				'User info provider is not configured. Call setUserInfoProvider() to provide user information retrieval logic.'
			);
		}
		...
		if (errors.length > 0) {
			...
			if (serverConfig.environment === 'production') {
				this.securityLogger.error(
					'CRITICAL: Server cannot start with missing required providers in production environment'
				);
				throw new Error(errorMessage);
			} else {
				// In non-production, log warning
				this.securityLogger.warn(
					'WARNING: Server is starting with incomplete configuration. This is only allowed in non-production environments.'
				);
	...
	private setupMiddleware() {
		...
		if (!sessionConfig.store && serverConfig.environment === 'production') {
			this.securityLogger.error(
				'CRITICAL: Using in-memory session store in production. Configure REDIS_URL environment variable for session durability and scalability!'
			);
	...
	private handleAuthorizationServerMetadata(req: Request, res: Response) {
		const metadata = {
			...
			registration_endpoint: this.config.registrationEndpoint
				? `${this.config.baseUrl}${this.config.registrationEndpoint}`
				: undefined,
			introspection_endpoint: this.config.introspectionEndpoint
				? `${this.config.baseUrl}${this.config.introspectionEndpoint}`
				: undefined,
			userinfo_endpoint: this.config.userInfoEndpoint
				? `${this.config.baseUrl}${this.config.userInfoEndpoint}`
				: undefined,
	...
	private async handleLogin(req: Request, res: Response) {
		try {
			...
			this.securityLogger.info('Processing login submission', {
				...
				cookies: Object.keys(cookies),
				hasCsrfCookie: !!(
					cookies['__Host-mcp.x-csrf-token'] ||
					cookies['__Secure-mcp.x-csrf-token'] ||
					cookies['mcp-csrf-token']
				),
			...
			if (!email || !password) {
				...
				return res.redirect(
					`${this.config.loginEndpoint}?error=missing_credentials&return_url=${encodeURIComponent(
						normalizedReturnUrl
					)}`
				);
			}
			...
			if (!user) {
				const normalizedReturnUrl = this.normalizeReturnUrl(returnUrl);
				return res.redirect(
					`${this.config.loginEndpoint}?error=invalid_credentials&return_url=${encodeURIComponent(
						normalizedReturnUrl
					)}`
				);
			}
			...
			if (session.oauthParams) {
				...
				this.securityLogger.debug(
					`Redirecting to authorization with restored OAuth params: ${authUrl.toString()}`
				);
		...
		try {
			...
			if (client.clientType === 'public') {
				...
				if (
					...
					(serverConfig.environment === 'production' && params.code_challenge_method !== 'S256')
				) {
	...
	private handleAuthorizationConsent(req: Request, res: Response) {
		try {
			const { client_id, redirect_uri, scope, state, code_challenge, code_challenge_method, user_consent } =
				req.body;
	...
	private async handleUserInfo(req: Request, res: Response): Promise<void> {
		try {
			...
			if (!validation.valid) {
				const resourceMetadataUrl = `${this.config.baseUrl}/.well-known/oauth-protected-resource`;
				const authError = BaseErrorHandler.createAuthError(
					'invalid_token',
			...
			const hasUserInfoScope = validation.scopes?.some(
			...
			if (!hasUserInfoScope) {
				...
				const authError = BaseErrorHandler.createAuthError(
					'insufficient_scope',
					'Token lacks required scope (openid/profile/email)'
				);
				return this.errorHandler.handleAuthError(res, authError, resourceMetadataUrl);
			}
			...
			<body>
				<div class="card">
					<div class="header">
						${
							client.logoUri
								? `<img src="${escapeHtml(client.logoUri)}" alt="${escapeHtml(
										client.clientName
								  )}" class="app-icon">`
								: ''
						}
	...
	private buildSecurityNotice(clientName: string): string {
		return `
			<div class="warning">
				<strong>⚠️ Security Notice:</strong> Only authorize applications you trust. This will give ${escapeHtml(
					clientName
				)} access to the permissions listed above.
			</div>
		`;
	}
