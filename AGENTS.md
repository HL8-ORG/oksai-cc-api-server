...
-   `qauzy-backup`目录下放置了旧的项目代码，我们将参考这些代码和代码组织管理结构进行开发，重新构建一个多租户的 SAAS 服务端应用。对 `backup` 目录下的旧项目代码进行简化，但是**不改动** `backu` 目录的代码结构和代码内容（注释除外）
-   新开发的功能和代码使用 `@oksai` 作为包名前缀
-   `libs` 目录用于存放新开发的代码，对齐 `qauzy-backup` 目录下的包和代码管理结构
